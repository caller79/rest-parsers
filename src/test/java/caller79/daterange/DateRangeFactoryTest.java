...
public class DateRangeFactoryTest {
    ...
    public void testBasicNumericParsing() {
        ...
        Assert.assertEquals(range.getStart(serverTimezone).getEpochSecond(), start);
        Assert.assertEquals(range.getEnd(serverTimezone).getEpochSecond(), start + 24 * 3600 * 30);
        ...
        Assert.assertTrue(range.overlaps(eventDate, eventDate.plus(1, ChronoUnit.DAYS), eventZone));
        Assert.assertFalse(range.overlaps(eventDate.minus(1, ChronoUnit.DAYS), eventDate, eventZone));
    ...
    public void testBasicDateParsing() {
        ...
        Assert.assertEquals(range.getStart(serverTimezone).getEpochSecond(), start);
        Assert.assertEquals(range.getEnd(serverTimezone).getEpochSecond(), end);
    ...
    @Test
    public void testChristmasInTokyo() {
        ZoneId serverTimezone = ZoneId.of("UTC");
        LocalDateTime todayInUTC = LocalDateTime.of(2021, 12, 24, 18, 30, 0);
        Instant now = todayInUTC.toInstant(serverTimezone.getRules().getOffset(todayInUTC));
        ...
        // Since the event is in Tokyo, even though now it UTC is 24th of December 18:30 in the evening, in Tokyo it is already
        // Christmas day, 3:30 am.
        Assert.assertTrue(christmas.overlaps(eventStart, eventEnd, eventTimezone));
    }
}
